...
export function formatResults(results: AnalysisResult, showStats: boolean = true): string {
  ...
  if (results.unusedImports.length > 0) {
    output += chalk.red(`❌ Found ${results.unusedImports.length} unused import${results.unusedImports.length === 1 ? '' : 's'}:\n`);
    for (const imp of results.unusedImports.slice(0, 10)) {
      output += chalk.gray(`  - ${imp.file}:${imp.line + 1} - `) + 
  ...
  if (results.unusedFiles.length > 0) {
    output += chalk.red(`❌ Found ${results.unusedFiles.length} unused file${results.unusedFiles.length === 1 ? '' : 's'}:\n`);
    for (const file of results.unusedFiles.slice(0, 10)) {
      output += chalk.gray(`  - ${file.path} `) +
  ...
  if (results.unusedDependencies.length > 0) {
    output += chalk.red(`❌ Found ${results.unusedDependencies.length} unused dependenc${results.unusedDependencies.length === 1 ? 'y' : 'ies'}:\n`);
